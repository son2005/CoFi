// https://app.codesignal.com/challenge/i6w5YHq3DbyCJ4oia
/*
...
function ratingThreshold(threshold, ratings) {
    /**
     * We have threshold like 3.5
     * ratings = [[3, 4],       // avg is 3.5
           [3, 3, 3, 4],        // avg is 3.25   <- reviewed
           [4]]                 // avg is 4
     * Result is 1
     */
    result = []
    for(i in ratings) {
        total = 0
        ratings[i].map(a => total += a)
        if (total / ratings[i].length < threshold) result.push(+i)
    }
    return result
}
