...
*/
// https://app.codesignal.com/challenge/EsW48xAgRdqg24eeq
function launchSequenceChecker(systemNames, stepNumbers) {
  const last = {};
  const n = systemNames.length;
  for (let i = 0; i < n; i++) {
    if (last[systemNames[i]] && last[systemNames[i]] >= stepNumbers[i]) {
      return false;
    }
    last[systemNames[i]] = stepNumbers[i];
  }
  return true;
}
