...
*/
// https://app.codesignal.com/challenge/fxauwMgou9oeg9riL
function campusCup(emails) {
    // Count number of schools
    var schools = {}
    for(var i in emails) {
        var name = emails[i].split`@`[1]
        schools[name] = schools[name] || 0
        schools[name]++
    }
    // Split results into 5 arrays correspondence to 5 bonus score
    arr = [[],[],[],[],[]]
    for(k in schools) {
        var i = 4
        if(schools[k] >= 25) i = 0
        else if(schools[k] >= 15) i = 1
        else if(schools[k] >= 10) i = 2
        else if(schools[k] >= 5) i = 3
        arr[i].push(k)
    }
    // Merge and sort results
    var r = []
    return arr.map( (a,i) => r = [...r, ...arr[i].sort()]) && r
}
