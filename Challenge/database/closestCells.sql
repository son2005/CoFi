...
https://codesignal.s3.amazonaws.com/tasks/closestCells/img/example.png?_tm=1582159535157

*/

-- https://app.codesignal.com/challenge/DQ6gJCJmi6Qs2CiCE
CREATE PROCEDURE closestCells()
SELECT id, (
    SELECT id
    FROM positions
    WHERE id != p.id
    ORDER BY ST_DISTANCE(POINT(x, y), POINT(p.x, p.y))
    LIMIT 1
) t
FROM positions p
