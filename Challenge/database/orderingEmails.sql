...
-- SELECT id, email_title, IF(size >= @a, CONCAT(FLOOR(size / @a), " Mb"), 
--         CONCAT(IF(size >= @b, FLOOR(size / @b), 0), " Kb")) short_size
-- FROM (
--     SELECT *, @b := 1024, @a := @b*@b
--     FROM emails
--     ORDER BY 3 DESC
-- ) e
SELECT id, email_title, IF(m > 0, CONCAT(m, " Mb"), CONCAT(k, " Kb")) short_size
FROM (
    SELECT *, @a := 1024, FLOOR(size / @a) k, FLOOR(size / (@a*@a)) m
    FROM emails
    ORDER BY 3 DESC
) e
