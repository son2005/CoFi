...
https://codesignal.s3.amazonaws.com/tasks/placesOfInterestPairs/img/ex.png?_tm=1582159667863
*/
-- https://app.codesignal.com/challenge/dnk9nv4KsJ46gqCtP
CREATE PROCEDURE placesOfInterestPairs()
SELECT a.name place1, b.name place2
FROM sights a, sights b
WHERE a.name < b.name
-- && ST_DISTANCE(POINT(a.x, a.y), POINT(b.x, b.y)) < 5
&& POW(POW(a.x - b.x, 2) + POW(a.y - b.y, 2), 0.5) < 5
ORDER BY 1, 2
