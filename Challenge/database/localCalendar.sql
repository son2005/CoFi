...
*/
-- https://app.codesignal.com/challenge/HefxN69cdX7yHQySh
CREATE PROCEDURE localCalendar()
-- add 1 hour
-- SELECT DATE_ADD("2012-12-12 04:04:00", INTERVAL 1 HOUR)

-- 12 hours format
-- SELECT DATE_FORMAT("2012-12-12 04:04:00", "%Y-%m-%d %r")

SELECT event_id,
IF(s.hours = 24,
    DATE_FORMAT(DATE_ADD(e.date, INTERVAL s.timeShift MINUTE), "%Y-%m-%d %H:%i"),
    DATE_FORMAT(DATE_ADD(e.date, INTERVAL s.timeShift MINUTE), "%Y-%m-%d %h:%i %p")) formatted_date
