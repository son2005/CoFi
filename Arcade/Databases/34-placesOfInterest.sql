...
SELECT country, SUM(a) adventure_park, SUM(b) golf, SUM(c) river_cruise, SUM(d) kart_racing
FROM (
        SELECT country, IF(leisure_activity_type = "Adventure park", number_of_places, 0)   a, IF(leisure_activity_type = "Golf", number_of_places, 0) b,
    IF(leisure_activity_type = "River cruise", number_of_places, 0) c,
    IF(leisure_activity_type = "Kart racing", number_of_places, 0) d
