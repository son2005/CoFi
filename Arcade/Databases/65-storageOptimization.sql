...
SELECT id, name column_name, value
FROM (
    (SELECT id, "name", name value FROM workers_info WHERE name IS NOT NULL)
    UNION
    (SELECT id, "date_of_birth", date_of_birth value FROM workers_info WHERE date_of_birth IS NOT NULL)
    UNION
    (SELECT id, "salary", salary value FROM workers_info WHERE salary IS NOT NULL)
) a
